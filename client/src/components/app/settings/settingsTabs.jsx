...
export default () => {
    ...
    return (
        <div>
            <ul role="tablist" className="hidden max-w-screen-xl border-l flex-col gap-y-3 text-sm sm:flex">
                {
                    tabItems.map((item, idx) => (
                        <li key={idx} className={`border-l-2 ${selectedItem == idx ? "border-indigo-400 text-indigo-400" : "border-white text-gray-500"}`}>
                            <button
                                role="tab"
                                aria-selected={selectedItem == idx ? true : false}
                                aria-controls={`tabpanel-${idx + 1}`}
                                className="py-2.5 px-4 rounded-lg duration-150 hover:text-indigo-400 hover:bg-gray-50 active:bg-gray-100 font-medium"
